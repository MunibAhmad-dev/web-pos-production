                    ...
                    return (
                      <div key={`po-${po.id}`} className="rounded-xl border border-border overflow-hidden bg-card shadow-sm">
                        ...
                        <div className="flex items-center justify-between px-4 pt-3 pb-2 gap-2">
                          ...
                          <div className="flex items-center gap-2 shrink-0">
                            <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                              <CalendarDays size={9} />{fmtDate(po.date_created)}
                            </span>
                            <button
                        ...
                        <div className="grid grid-cols-3 px-4 pb-2 text-center">
                          ...
                          <div>
                            <p className={cn('text-[9px] font-black uppercase tracking-widest', po.remaining > 0.5 ? 'text-rose-600' : 'text-emerald-600')}>Remaining</p>
                        ...
                        <div className="flex items-center gap-1.5 px-4 pb-3 flex-wrap">
                          ...
                          <div className="flex items-center gap-1.5 ml-auto">
                            {vendor.phone && (
                              <button onClick={() => waPoStatus(po)} title="WhatsApp"
                            ...
                            <button onClick={() => printInvoice(poInvoiceData(po))} title="Print"
                            ...
                            {po.status !== 'Cancelled' && (
                              <button onClick={() => cancelPO(po)} title="Cancel order"
                        ...
                        {/* Expanded detail: items table + payments on PO + returns */}
                        {isExpanded && (
                          <div className="border-t border-border/40 bg-muted/10 px-4 py-3 space-y-3">
                                ...
                                <div className="space-y-1.5">
                                  {po.items.map((it, idx) => (
                                    <div key={idx} className="flex items-center text-xs gap-2">
                                      ...
                                      <span className="text-muted-foreground shrink-0 tabular-nums w-20 text-right">{fmtPKR(it.cost_price || it.purchase_price)}</span>
                                      <span className="font-bold shrink-0 tabular-nums w-24 text-right">{fmtPKR(Number(it.cost_price || it.purchase_price || 0) * Number(it.quantity))}</span>
                                    </div>
                                  ))}
                                </div>
                            ...
                            {po.linkedPayments?.length > 0 && (
                              <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-2">Payments on this Order</p>
                                <div className="space-y-1.5">
