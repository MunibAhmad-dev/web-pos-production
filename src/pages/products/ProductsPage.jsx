...
export default function ProductsPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      totalStock: products.reduce((s, p) => s + Math.max(0, p.stock || 0), 0),
  ...
  const analytics = useMemo(() => {
    ...
    const totalStockValue = products.reduce((s, p) => s + Math.max(0, p.stock || 0) * (p.purchase_price || 0), 0);
    const totalRetailValue = products.reduce((s, p) => s + Math.max(0, p.stock || 0) * (p.price || 0), 0);
    ...
    const categoryBreakdown = Object.entries(
      products.reduce((acc, p) => {
        ...
        acc[cat].value += Math.max(0, p.stock || 0) * (p.purchase_price || 0);
        acc[cat].retail += Math.max(0, p.stock || 0) * (p.price || 0);
        acc[cat].count++;
        return acc;
      }, {})
