...
export default function VendorFormModal({ open, onClose, onSubmit, vendor }) {
  ...
  useEffect(() => {
    setForm(
      vendor
        ? { name: vendor.name || '', phone: vendor.phone || '', email: vendor.email || '', address: vendor.address || '', notes: vendor.notes || '' }
        : emptyForm
    );
    setError('');
  }, [vendor, open]);
